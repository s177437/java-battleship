 ...
 */
class GameLogic {
    private Map<String, List<String>> scores = new HashMap<>();

    void play(List<List<String>> board, List<String> playerlist, int boardid, long speed, String player) throws InterruptedException {
        ...
        } else if (board.get(row).get(column).equals("**") || board.get(row).get(column).equals("--")) {
